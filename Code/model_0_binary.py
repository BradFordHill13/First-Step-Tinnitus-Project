...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 30 16:07:44 2020
...
def assign_tinnitus_phenotype(row, severity_level):
    ...
    if np.isnan(tinnitus_severity_score): # if we add or tinnitus_severity_score != 11 then we select severe tinnitus 
...
def tinnitus_extraction_to_phenofile(output_file, sample_file, extraction_file, severity_level):
    ...
    # To balance controls and cases
    update_output = read_samples(sample_file)
    update_output['tinnitus'] = np.nan
    control_to_keep = output[output['tinnitus'] == 0].sample(n=output['tinnitus'].value_counts()[1]) # choose by random which control we keep
    for index, value in output.iterrows():
        if index in control_to_keep.index or value[0] == 1.0:
            update_output.loc[index, 'tinnitus'] = output.loc[index][0]
    print(" ")        
    print("Check the balance controls and cases")       
    print(update_output['tinnitus'].value_counts()) # to control it works well
    print(" ")
    
    valid = update_output.iloc[:,0].notna().sum() # count number of non-NA results
    n_cases = len(update_output[(update_output["tinnitus"]>0)])
    n_controls = len(update_output[(update_output["tinnitus"]==0)])
    ...
    update_output.fillna(-999, inplace=True) # replace NAs according to BGENIE convention
    update_output.replace(-1, -999, inplace=True) # -1 is also equivalent to an NA, for tinnitus
    # output
    ...
    update_output.to_csv(output_file, sep=' ', index = False)
...
def main():
    ...
    severity_level = "any"
    ###########################################################################
    output_file = "./output/phenofile_tinnitus_model_0_binary_Col1_balance0and1_"+severity_level+".csv"
    #output_file ="./test_output/phenofile_tinnitus_model_0_binary_balance0and1_"+severity_level+".csv"
    ...
    #extraction_file = "test_input/model_0_binary_input.csv"
